...
{
float a=6.28319*uv.x;
float b=3.14159*uv.y;
float x=sin(a)*sin(b);
float y=cos(b);
float z=cos(a)*sin(b);
return float3(z,y,x);
...
{
return fract(sin(dot(st,float2(12.9898,78.233)))*43758.5);
}
...
float param_1=-R.z;
float2 uv;
uv.x=_atan2(param,param_1)-1.5708;
uv.y=acos(R.y);
uv/=float2(6.28319,3.14159);
return uv;
}
...
float shiftAngle=3.14159/(*sc_set2.UserUniforms).baseTexSize.y;
