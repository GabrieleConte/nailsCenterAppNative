...
{
float add0=1.0-fast::max(abs(sign(num)),0.999);
return num+add0;
}
...
texCoord=fast::clamp(texCoord,float2(0.002),float2(0.998));
...
float MIPf=fast::min(log2(fast::max((angleOfInterest-0.78539)/0.04909,1.0)),8.0);
...
float complimentaryAngle=fast::max(angleOfInterest-1.57079,0.0);
float complimentaryMixAmount=0.5*smoothstep(0.0,1.57079,complimentaryAngle);
float complimentaryMIPf=fast::min(log2(fast::max((2.35617+(1.57079-complimentaryAngle))/0.04909,1.0)),8.0);
...
bool param_38=(int(SC_USE_UV_TRANSFORM_baseTex)!=0);
float3x3 param_39=(*sc_set2.UserUniforms).baseTexTransform;
...
float4 l9_5=l9_4;
float4 finColor=l9_4;
finColor=mix(borderSample,finColor,float4(isInsideNDC));
