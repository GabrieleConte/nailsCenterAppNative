...
{
return pow((rgb+float3(0.055))*0.947867,float3(2.4));
}
float3 rgb2lab(thread float3& rgb)
{
float3 param=rgb;
rgb=rgb2srgb(param);
float3 xyz=float3x3(float3(0.433953,0.212671,0.017758),float3(0.376219,0.71516,0.109477),float3(0.189828,0.072169,0.872766))*rgb;
float3 f=pow(xyz,float3(0.333333));
...
{
return (pow(srgb,float3(0.416667))*1.055)-float3(0.055);
}
...
float3 param=float3x3(float3(3.07994,-0.921234,0.05289),float3(-1.53715,1.87599,-0.204041),float3(-0.542783,0.045244,1.15115))*xyz;
