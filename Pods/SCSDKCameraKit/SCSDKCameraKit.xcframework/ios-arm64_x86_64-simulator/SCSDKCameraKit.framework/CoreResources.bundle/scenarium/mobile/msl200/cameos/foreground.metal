...
//ubo float UserUniforms 2:6:388 {
...
{
result=float4(mix(bg,bg/((float3(1.0)-fg)+float3(0.01)),float3(alpha)),alpha);
}
#else
{
#if (OVERLAY_BLENDING)
...
if (fg[i]>0.5)
{
blendColor_2[i]=bg[i]/((2.0*(1.0-fg[i]))+0.00392157);
}
else
{
blendColor_2[i]=1.0-((1.0-bg[i])/((2.0*fg[i])+0.00392157));
}
i++;
continue;
}
